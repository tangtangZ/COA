...
public class Memory {
    ...
    public void seg_load(int segIndex) {
        ...
        /*if(PAGE){
            ...
            int base_num=Integer.parseInt(String.copyValueOf(base),2);//得到线性地址
            page_load((Integer.parseInt(inneradder,2)+base_num)/4096);//线性地址的高20位为虚拟页号
        }else {
            data= disk.read(base.toString(),(int)Math.pow(2,20));
            write("00000000000000000000000000000000",(int)Math.pow(2,20),data);}*/
        if(!PAGE){
            data=disk.read(String.valueOf(base),(int)Math.pow(2,20));
            write(base.toString(),(int)Math.pow(2,20),data);
        }
    ...
    public void page_load(int vPageNo) {
        // TODO: paste your code here
        int lineadder=vPageNo*4096;
        //从磁盘读数据；
        char [] data=disk.read(transformer.intToBinary(String.valueOf(lineadder)),4096);
        int pageNo=-1;
        ...
        pageItem.pageFrame=transformer.intToBinary(String.valueOf(pageNo)).substring(12,32).toCharArray();
