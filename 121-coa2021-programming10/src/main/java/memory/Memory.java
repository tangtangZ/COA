...
public class Memory {
    ...
    public void seg_load(int segIndex) {
        ...
        getSegDescriptor(segIndex).base="00000000000000000000000000000000".toCharArray();
        getSegDescriptor(segIndex).limit="11111111111111111111".toCharArray();
        char data[]={};
        char base[]=getBaseOfSegDes(segIndex);
        if(PAGE){
            String inneradder=MMU.getMMU().GETLOGICadder().substring(16,48);
            int base_num=Integer.parseInt(String.copyValueOf(base),2);
            page_load((Integer.parseInt(inneradder,2)+base_num)/4096);
        }else {
        data= disk.read(base.toString(),(int)Math.pow(2,20));
        write("00000000000000000000000000000000",(int)Math.pow(2,20),data);}
        GDT[segIndex].base="00000000000000000000000000000000".toCharArray();
        GDT[segIndex].limit="11111111111111111111".toCharArray();
        GDT[segIndex].validBit=true;
    }
    ...
    public void page_load(int vPageNo) {
        int line=vPageNo*(int)Math.pow(2,12);
        char data[]=disk.read(transformer.intToBinary(String.valueOf(line)),(int)Math.pow(2,12));
        int pageNo=-1;
        for(int i=0;i< 8*1024 ;i++){
            if(pageValid[i]==false){
                pageNo=i;
                break;
            }
        }
        write(transformer.intToBinary(String.valueOf(pageNo*4096)),4096,data);
        getPageItem(vPageNo).isInMem=true;
        getSegDescriptor(pageNo).granularity=true;
        pageValid[pageNo]=true;
        pageTbl[vPageNo].pageFrame=transformer.intToBinary(String.valueOf(pageNo)).substring(12,32).toCharArray();
    }
